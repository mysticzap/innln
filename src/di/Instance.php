 ...
 *
 * ```php
 * $container = new \innln\di\Container;
 * $container->set('cache', [
 *     'class' => 'innln\caching\DbCache',
 *     'db' => Instance::of('db')
 * ]);
 * $container->set('db', [
 ...
 *     {
 *         parent::init();
 *         $this->db = Instance::ensure($this->db, 'innln\db\Connection');
 *     }
 * }
 * ```
 *
...
class Instance
{
     ...
     *
     * If you do not specify a container, the method will first try `Innln::$app` followed by `Innln::$container`.
     *
     * For example,
     *
     * ```php
     ...
     * If null, the method will first try `Innln::$app` then `Innln::$container`.
    ...
    public function get($container = null)
    {
        ...
        if (Innln::$app && Innln::$app->has($this->id)) {
            return Innln::$app->get($this->id);
        }

        return Innln::$container->get($this->id);
    }
